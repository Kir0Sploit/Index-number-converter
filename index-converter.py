...
def convert_to_indices(input_string):
    indices = []
    for char in input_string:
        if char in char_index_mapping:
            indices.append(f'a[{char_index_mapping[char]}]')
    return '+'.join(indices)

user_input = input("Enter a string: ")
result_indices = convert_to_indices(user_input)
print(f"Converted: {result_indices}")
